...
function update_balance() {
    var due = $("#total").html().replace("SAR", "") - $("#paid").val().replace("SAR", "");
...
function update_price() {
    ...
    var price = row.find('.cost').val().replace("SAR", "") * row.find('.qty').val();
    price = roundNumber(price, 2);
    isNaN(price) ? row.find('.price').html("N/A") : row.find('.price').html("SAR" + price);
    update_total();
}
